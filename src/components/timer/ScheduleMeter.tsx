...
export const ScheduleMeter = () => {
  ...
  return (
    <section
      className="w-full max-w-lg shrink-0 mb-1 sm:mb-2 md:mb-3"
    ...
    >
      <div className="flex items-center justify-between gap-3 mb-2">
        ...
        <p className="text-[11px] sm:text-xs text-right text-white/70">
          Next: <span className="text-white">{nextLabel}</span>
        </p>
      </div>

      <div className="flex gap-1.5 sm:gap-2" role="list" aria-label="Cycle steps">
        {segments.map((segment, index) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={cn(
                  'truncate text-center text-[10px] sm:text-[11px] font-semibold uppercase tracking-wide transition-colors',
                  isCurrent
                    ? 'text-white'
                    : isComplete
                      ? 'text-white/70'
      ...
      <p className="mt-2 text-xs sm:text-sm text-white/75 text-center">
        {currentLabel}
      </p>
    </section>
  );
